...
int main(int argc, char *argv[])
{
	...
	if (argc<3) 
		error("Error in arguments: ManFut <presupost> <fitxer_jugadors>");
...
void CalcularEquipOptim(long int PresupostFitxatges, PtrJugadorsEquip MillorEquip)
{
	...
	TEquip primerEquip, ultimEquip, first, end;
	int MaxPuntuacio;
	...
	for (int i = 0; i < numero_threads; i++){
		if (i == 0){
			...
			args_arr[i].first = args_arr[i-1].end;
		}
	...
	if (numero_threads == 0){
		...
		main_args.first = args_arr[numero_threads - 1].end;
	}
	main_args.end = main_args.first + combinacions_per_thread;
	main_args.presupost = PresupostFitxatges;
	...
	for(int i = 0; i < numero_threads; i++){
		...
		if(pthread_join(threads[i], (void **) &retorn_thread) != 0){
			error("Error al fer un join");
		}
		retorns[i] = *retorn_thread;
		free(retorn_thread);
		if (PuntuacioEquip(retorns[i])>MaxPuntuacio)
		{
			MaxPuntuacio=PuntuacioEquip(retorns[i]);
			memcpy(MillorEquip,&retorns[i],sizeof(TJugadorsEquip));
		}
	}
}
...
void *calcular_millor_parcial(void *argvs){
	...
	TEquip first = args.first;
	TEquip end = args.end;
	int PresupostFitxatges = args.presupost;
	...
	sprintf (cad,"Evaluating from %llXH to %llXH. Evaluating %lld teams...\n",first,end,end-first);
	write(1,cad,strlen(cad));
	...
	TJugadorsEquip *return_equip;
	return_equip = (TJugadorsEquip *) malloc(sizeof(TJugadorsEquip));
	*return_equip = MillorEquip;
	return return_equip;
}
